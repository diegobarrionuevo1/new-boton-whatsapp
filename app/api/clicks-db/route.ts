...
const getNumbers = async (): Promise<Record<BusinessType, string[]>> => {
    ...
    // Usar la configuración por defecto si no hay datos en KV
    return {
        Hero: [
...
export async function GET(request: NextRequest) {
    try {
        ...
        // Obtener datos del negocio desde KV
        const statsKey = `stats-${businessKey}`;
        const businessData = await kv.get(statsKey) || { 
            clickCount: 0, 
            uniqueUsers: [], 
            dailyClicks: {} 
        };
...
export async function POST(request: NextRequest) {
    try {
        ...
        // Obtener datos actuales desde KV
        const statsKey = `stats-${businessKey}`;
        const businessData = await kv.get(statsKey) || { 
            clickCount: 0, 
            uniqueUsers: [], 
        ...
        // Guardar datos actualizados en KV
        await kv.set(statsKey, businessData);
